...
const COOLDOWN_KEY = "north-eastern-lawn-magic-link-cooldown";
...
export default function Login() {
  ...
  const [cooldown, setCooldown] = useState(() => typeof window === "undefined" ? 0 : readCooldown());

  useEffect(() => {
    const interval = window.setInterval(() => setCooldown(readCooldown()), 1000);
    return () => window.clearInterval(interval);
  }, []);

  function beginCooldown(seconds = COOLDOWN_SECONDS) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (authError) {
      if (authError.message.toLowerCase().includes("rate limit")) {
        beginCooldown();
        setError(`A sign-in email was requested recently. Please wait ${COOLDOWN_SECONDS} seconds before trying again.`);
        return;
      }
